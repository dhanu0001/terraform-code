        ...
        try {
          ...
          if (!result.ok) {
            ...
            await this.stitcher.stitch(
              /* @__PURE__ */ new Set([catalogModel.stringifyEntityRef(unprocessedEntity)])
            );
...
class Stitcher {
  ...
  async stitch(entityRefs) {
    for (const entityRef of entityRefs) {
      try {
        ...
        this.logger.error(
          `Failed to stitch ${entityRef}, ${errors.stringifyError(error)}`
        );
      }
    }
  }
  async stitchOne(entityRef) {
    ...
    await this.database.batchInsert("search", searchEntries, BATCH_SIZE$1);
  }
}
...
class AuthorizedEntitiesCatalog {
  ...
  async entityAncestry(entityRef, options) {
    const rootEntityAuthorizeResponse = (await this.permissionApi.authorize(
      [{ permission: pluginCatalogCommon.catalogEntityReadPermission, resourceRef: entityRef }],
      { token: options == null ? void 0 : options.authorizationToken }
    ))[0];
...
class AuthorizedLocationService {
  ...
  async createLocation(spec, dryRun, options) {
    const authorizationResponse = (await this.permissionApi.authorize(
      [{ permission: pluginCatalogCommon.catalogLocationCreatePermission }],
      { token: options == null ? void 0 : options.authorizationToken }
    ))[0];
  ...
  async listLocations(options) {
    const authorizationResponse = (await this.permissionApi.authorize(
      [{ permission: pluginCatalogCommon.catalogLocationReadPermission }],
      { token: options == null ? void 0 : options.authorizationToken }
    ))[0];
    if (authorizationResponse.result === pluginPermissionCommon.AuthorizeResult.DENY) {
      return [];
  ...
  async deleteLocation(id, options) {
    const authorizationResponse = (await this.permissionApi.authorize(
      [{ permission: pluginCatalogCommon.catalogLocationDeletePermission }],
      { token: options == null ? void 0 : options.authorizationToken }
    ))[0];
    if (authorizationResponse.result === pluginPermissionCommon.AuthorizeResult.DENY) {
      throw new errors.NotAllowedError();
