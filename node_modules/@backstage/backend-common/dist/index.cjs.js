...
class RootContext {
  constructor() {
    this.abortSignal = new FakeAbortSignal();
...
function getCertificateExpiration(cert, logger) {
  try {
    const crt = forge__default["default"].pki.certificateFromPem(cert);
    return crt.validity.notAfter.getTime() - Date.now();
  } catch (error) {
    logger == null ? void 0 : logger.warn(`Unable to parse self-signed certificate. ${error}`);
    return 0;
  }
}
