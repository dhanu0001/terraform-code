...
function convertToHumanDuration(config, key) {
...
function readDuration(config, key) {
  return typeof config.get(key) === "string" ? luxon.Duration.fromISO(config.getString(key)) : convertToHumanDuration(config, key);
}
...
class TaskScheduler {
  ...
  forPlugin(pluginId) {
    const databaseFactory = lodash.once(async () => {
      ...
      if (!((_a = databaseManager.migrations) == null ? void 0 : _a.skip)) {
